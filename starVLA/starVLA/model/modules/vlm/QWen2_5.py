...
def _resolve_writable_hf_cache_dir() -> Optional[str]:
    candidates = [
    ...
    workspace_cache = Path(__file__).resolve().parents[5] / ".cache" / "huggingface"
    candidates.append(str(workspace_cache))
    ...
    for candidate in candidates:
        ...
        try:
            ...
            test_file.unlink()
            hf_home = str(candidate_path.parent if candidate_path.name in {"hub", "transformers"} else candidate_path)
            hf_hub_cache = str(candidate_path if candidate_path.name == "hub" else candidate_path / "hub")
            transformers_cache = str(candidate_path if candidate_path.name == "transformers" else candidate_path / "transformers")
...
class _QWen_VL_Interface(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        cache_dir = _resolve_writable_hf_cache_dir() if isinstance(model_id, str) and not Path(model_id).exists() else None
