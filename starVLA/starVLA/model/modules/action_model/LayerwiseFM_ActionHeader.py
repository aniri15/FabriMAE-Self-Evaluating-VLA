...
class LayerwiseFlowmatchingActionHead(nn.Module):
    ...
    def forward(self, vl_embs_list: list, actions: torch.Tensor, state: torch.Tensor = None):
        ...
        for layer_idx, layer in enumerate(self.model.transformer_blocks):
            model_output, attn_weight = layer(
                hidden_states=model_output, # query
                encoder_hidden_states=vl_embs_list[layer_idx],  # key, value # Use layer-specific vl_embs
                temb=temb,
            )
        ...
        return loss, tuple(attn_weights_list)
    ...
    def predict_action(self, vl_embs_list: list, state: torch.Tensor = None) -> torch.Tensor:
        ...
        for t in range(num_steps):
            ...
            attn_timestamp.append(tuple(attn_weights_list))
        return actions, tuple(attn_timestamp)
