...
# 
    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.autocast("cuda", dtype=torch.float32):
            ...
            action_loss, attn_weights = self.action_model(vl_embs_list_repeated, actions_target_repeated, state_repeated)  # (B, chunk_len, action_dim)

        # Filter attention weights
        visual_attn = tuple(attn_weight[:, :, -8: , visual_tokens] for attn_weight in attn_weights)
        text_attn = tuple(attn_weight[:, :, -8:, text_tokens] for attn_weight in attn_weights)
    ...
    def predict_action( # TODO align  predict_action with forward, make api more flexible
        ...
        **kwargs: str,
        ...
        # Step 4: Action Expert Forward and Loss
        with torch.autocast("cuda", dtype=torch.float32):
            pred_actions, attn_weights = self.action_model.predict_action(vl_embs_list, state)  # (B, chunk_len, action_dim)

        normalized_actions = pred_actions.detach().cpu().numpy()
        visual_attn = tuple(tuple(attn_time_weight[:, :, -8: , visual_tokens[0]].detach().cpu().numpy() for attn_time_weight in attn_layer_weight) for attn_layer_weight in attn_weights) #[t, L, 1, H, num_action, num_embedding_token]
        text_attn = tuple(tuple(attn_time_weight[:, :, -8: , text_tokens[0]].detach().cpu().numpy() for attn_time_weight in attn_layer_weight) for attn_layer_weight in attn_weights) #[t, L, 1, H, num_action, num_embedding_token]
...
if __name__ == "__main__":
    ...
    model = Qwen_PI(cfg)
    # ckpt="<set PRETRAINED_CHECKPOINT>"
