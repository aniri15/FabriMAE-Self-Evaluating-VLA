...
class Attention(nn.Module):
    ...
    @nn.compact
    def __call__(self, xs, positions, attn_mask, kv_cache, attention_stats_mask):
        ...
        def compute_head_entropy(_):
            stats_probs = probs.astype(jnp.float32) * attention_stats_mask[:, None, None, :, :]
            stats_probs /= jnp.maximum(jnp.sum(stats_probs, axis=-1, keepdims=True), 1e-12)
            entropy = -jnp.sum(
                jnp.where(stats_probs > 0, stats_probs * jnp.log(jnp.maximum(stats_probs, 1e-12)), 0.0),
                axis=-1,
            )
            query_mask = jnp.any(attention_stats_mask, axis=-1).astype(jnp.float32)
            entropy = jnp.sum(entropy * query_mask[:, None, None, :], axis=-1)
            entropy /= jnp.maximum(jnp.sum(query_mask, axis=-1)[:, None, None], 1.0)
            return einops.rearrange(entropy, "B K G -> B (K G)")

        head_entropy = jax.lax.cond(
            jnp.any(attention_stats_mask),
            compute_head_entropy,
            lambda _: jnp.zeros((q.shape[0], self.configs[0].num_heads), dtype=jnp.float32),
            operand=None,
        )
...
class Block(nn.Module):
    ...
    def __call__(
        ...
        post_attn, kv_cache, head_entropy = attn(pre_attn, positions, attn_mask, kv_cache, attention_stats_mask)
        ...
        return xs, (kv_cache, head_entropy)
...
class Module(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        attention_stats_mask = jnp.zeros(mask.shape[:1] + mask.shape[2:], dtype=bool)
        embedded, (kv_cache, _) = self.layers(
            embedded, kv_cache, positions, mask, adarms_cond, attention_stats_mask, deterministic
        )
    ...
    def with_attention_stats(
        self,
        embedded: Sequence[at.Float[at.Array, "b _t _d"] | None],
        positions: at.Int[at.Array, "b t"],
        mask: at.Bool[at.Array, "b t s"],
        attention_stats_mask: at.Bool[at.Array, "b t s"],
        adarms_cond: Sequence[at.Float[at.Array, "b _d"] | None] | None = None,
        *,
        kv_cache: KVCache | None = None,
        deterministic: bool = True,
        ...
        embedded, (kv_cache, layer_head_entropy) = self.layers(
            embedded,
            kv_cache,
            positions,
            mask,
        ...
        return embedded, kv_cache, layer_head_entropy
