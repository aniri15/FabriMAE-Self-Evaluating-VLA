...
TASK_MAX_STEPS = {
    ...
    TaskSuite.LIBERO_GOAL_SWAP: 520,
    TaskSuite.LIBERO_SPATIAL_SWAP: 520,
    TaskSuite.LIBERO_10_SWAP: 520,
    TaskSuite.LIBERO_OBJECT_SWAP: 520,
...
def setup_logging(cfg: GenerateConfig):
    ...
    cfg.local_log_dir = os.path.join(cfg.local_log_dir, 'libero_pro')
...
def run_episode(
    ...
    if cfg.task_suite_name in TASK_MAX_STEPS:
        ...
        log_message(f"Using max_steps={max_steps} for task suite '{cfg.task_suite_name}'", log_file)
    else:
                    ...
                    if attentions is not None:
                        ...
                        if isinstance(attentions, tuple):
                            ...
                            if cfg.save_attn_dir and task_id >= 0 and episode_idx >= 0:
                                save_single_attention_weight(
                                    cfg, attentions_cpu, task_id, episode_idx, query_idx,
                                    task_description, img, False, episode_number,
                                    num_patches=num_patches, num_prompt_tokens=num_prompt_tokens
                                )
                            del attentions_cpu
                            query_idx += 1
...
def run_task(
    ...
    for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
        ...
        # Save replay video
        save_rollout_video(
            cfg, replay_images, total_episodes, success=success, task_description=task_description, log_file=log_file, date_stamp=date_stamp, run_stamp=run_stamp
        )

        if success:
            saved_result['success'].append((task_id, episode_idx))
        else:
            saved_result['failure'].append((task_id, episode_idx))

        # Note: Attention data is already saved during episode execution
        # If we need to update success status in saved files, we could do it here
        # For now, success=False is saved initially, which is acceptable
    ...
    return total_episodes, total_successes, saved_result
...
def eval_libero(cfg: GenerateConfig) -> float:
    ...
    saved_result = {'success': [], 'failure': []}
    # for task_id in tqdm.tqdm(range(num_tasks)):
    for task_id in tqdm.tqdm(range(num_tasks)):
        total_episodes, total_successes, saved_result = run_task(
            cfg,
            task_suite,
            task_id,
            model,
    ...
    # Save Results
    result_path = os.path.join(cfg.save_attn_dir, "final_results.yaml") if cfg.save_attn_dir else os.path.join(cfg.local_log_dir, "final_results.yaml")
    with open(result_path, "w", encoding="utf-8") as f:
        yaml.dump(saved_result, f, allow_unicode=True)
