...
def update_auto_map(pretrained_checkpoint: str) -> None:
    ...
    current_auto_map = config.get("auto_map", {})
    if current_auto_map == expected_auto_map:
        print(f"Config at {config_path} already has correct auto_map. Skipping write.")
        return
...
def check_model_logic_mismatch(pretrained_checkpoint: str) -> None:
    ...
    # Find current files using absolute paths based on imported modules
    curr_files = {}
    
    # Method 1: Try to get paths from imported modules
    try:
        from prismatic.extern.hf import modeling_prismatic, configuration_prismatic
        curr_files["modeling_prismatic.py"] = os.path.abspath(modeling_prismatic.__file__)
        curr_files["configuration_prismatic.py"] = os.path.abspath(configuration_prismatic.__file__)
    except (ImportError, AttributeError):
        # Method 2: Fallback to searching from current file's directory
        current_file_dir = os.path.dirname(os.path.abspath(__file__))
        # Go up to openvla-oft_code directory
        codebase_root = os.path.abspath(os.path.join(current_file_dir, "../.."))
        prismatic_hf_dir = os.path.join(codebase_root, "prismatic", "extern", "hf")
        
        modeling_path = os.path.join(prismatic_hf_dir, "modeling_prismatic.py")
        config_path = os.path.join(prismatic_hf_dir, "configuration_prismatic.py")
        
        if os.path.exists(modeling_path):
            curr_files["modeling_prismatic.py"] = os.path.abspath(modeling_path)
        if os.path.exists(config_path):
            curr_files["configuration_prismatic.py"] = os.path.abspath(config_path)
        
        # Method 3: If still not found, try searching from current directory
        if "modeling_prismatic.py" not in curr_files or "configuration_prismatic.py" not in curr_files:
            for root, _, files in os.walk("./prismatic/"):
                for filename in ["modeling_prismatic.py", "configuration_prismatic.py"]:
                    if filename in files and filename not in curr_files:
                        curr_files[filename] = os.path.abspath(os.path.join(root, filename))

    # Check and handle each file
    for filename in ["modeling_prismatic.py", "configuration_prismatic.py"]:
        curr_filepath = curr_files.get(filename)
        if curr_filepath is None or not os.path.exists(curr_filepath):
            print(f"WARNING: `{filename}` is not found. Expected at: {os.path.join(os.path.dirname(os.path.abspath(__file__)), '../..', 'prismatic', 'extern', 'hf', filename)}")
            continue
...
def get_vla_action(
...
) -> Union[List[np.ndarray], Tuple[List[np.ndarray], Optional[float]], Tuple[List[np.ndarray], Optional[Any]]]:
    """
    ...
    Returns:
        ...
        Optional[float]: Uncertainty score when `return_uncertainty` is True
    """
    with torch.inference_mode():
        ...
        # Initialize token length variables
        ...
        if action_head is None:
            ...
            if return_uncertainty:
                ...
                if output_attentions:
                    action, _, uncertainty_score, attentions, num_patches, num_prompt_tokens = result
                else:
                    action, _, uncertainty_score = result
            else:
                #print(f"[DEBUG get_vla_action] Calling predict_action with output_attentions={output_attentions}, return_uncertainty=False")
                result = vla.predict_action(
                    **inputs,
                    unnorm_key=cfg.unnorm_key,
                    do_sample=False,
                ...
                #print(f"[DEBUG get_vla_action] predict_action returned: type={type(result)}, length={len(result) if isinstance(result, (tuple, list)) else 'N/A'}")
                if output_attentions:
                    #print(f"[DEBUG get_vla_action] Unpacking result with output_attentions=True, expecting 5 values")
                    action, _, attentions, num_patches, num_prompt_tokens = result
                    #print(f"[DEBUG get_vla_action] Unpacked: action shape={action.shape if hasattr(action, 'shape') else type(action)}, attentions={attentions}")
                else:
                    action, _ = result
        else:
            # Custom action head for continuous actions
            #print(f"[DEBUG get_vla_action] Calling predict_action with action_head, output_attentions={output_attentions}")
            result = vla.predict_action(
                **inputs,
                unnorm_key=cfg.unnorm_key,
                do_sample=False,
            ...
            #print(f"[DEBUG get_vla_action] predict_action (with action_head) returned: type={type(result)}, length={len(result) if isinstance(result, (tuple, list)) else 'N/A'}")
            if output_attentions:
                #print(f"[DEBUG get_vla_action] Unpacking result with output_attentions=True, expecting 5 values")
                action, _, attentions, num_patches, num_prompt_tokens = result
                #print(f"[DEBUG get_vla_action] Unpacked: action shape={action.shape if hasattr(action, 'shape') else type(action)}, attentions={attentions}")
    ...
    #print(f"[DEBUG get_vla_action] Before final return: output_attentions={output_attentions}, return_uncertainty={return_uncertainty}, attentions={attentions}")
    if output_attentions:
        if return_uncertainty:
            #print(f"[DEBUG get_vla_action] Returning 5 values: action_list, uncertainty_score, attentions, num_patches, num_prompt_tokens")
            return action_list, uncertainty_score, attentions, num_patches, num_prompt_tokens
        #print(f"[DEBUG get_vla_action] Returning 4 values: action_list, attentions, num_patches, num_prompt_tokens")
        return action_list, attentions, num_patches, num_prompt_tokens
    elif return_uncertainty:
        #print(f"[DEBUG get_vla_action] Returning 2 values: action_list, uncertainty_score")
        return action_list, uncertainty_score
    #print(f"[DEBUG get_vla_action] Returning 1 value: action_list")
    return action_list
...
def resolve_self_eval_output_dir(
    ...
    project_root = os.getenv(
        "OPENVLA_OFT_ROOT",
        os.getenv(
            "VLA_SELFAWARE_CP_ROOT",
            os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..")),
        ),
    )
    results_root = os.getenv(
        "OPENVLA_EVAL_RESULTS_ROOT",
        os.path.join(project_root, "eval_results"),
    )
    return os.path.join(
        results_root,
        benchmark,
