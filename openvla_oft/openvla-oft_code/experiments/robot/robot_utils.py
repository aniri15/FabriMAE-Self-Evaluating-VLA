...
def get_action(
...
) -> Union[List[np.ndarray], np.ndarray, Tuple[List[np.ndarray], Optional[float]], Tuple[List[np.ndarray], Optional[Any]]]:
    """
    ...
    Returns:
        ...
        Optional[float]: Uncertainty score when requested
    ...
    with torch.no_grad():
        if cfg.model_family == "openvla":
            #print(f"[DEBUG get_action] Calling get_vla_action with output_attentions={output_attentions}, return_uncertainty={return_uncertainty}")
            ...
            #print(f"[DEBUG get_action] get_vla_action returned: type={type(action)}, length={len(action) if isinstance(action, (tuple, list)) else 'N/A'}")
        else:
            raise ValueError(f"Unsupported model family: {cfg.model_family}")

    #print(f"[DEBUG get_action] Returning action: type={type(action)}, length={len(action) if isinstance(action, (tuple, list)) else 'N/A'}")
    # get_vla_action now returns (action_list, ...) where ... may include attentions, num_patches, num_prompt_tokens
    # This function just passes through the return value
    return action
